...
public class scary_maze extends JPanel implements Runnable, MouseMotionListener {
	...
	BufferedImage maze;
	final int frameWidth = 665;
	final int frameHeight = 437;
...
System.out.println(mouseColor);
	...
	private void scare() {
		...
		AudioClip sound = JApplet.newAudioClip(getClass().getResource("explosion.aif"));
		...
		showScaryImage("NuclurExplosion.jpg");
	}

	private void showScaryImage(String NuclurExplosion) {
		try {
			maze = ImageIO.read(getClass().getResource(NuclurExplosion));
		} catch (Exception e) {
			System.err.println("Couldn't find this image: " + NuclurExplosion);
		}
		repaint();
	}
